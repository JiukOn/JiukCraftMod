...
import net.mcreator.jiukonmod.procedures.OnearChaosQuandoAEntidadeMorreProcedure;
import net.mcreator.jiukonmod.procedures.OnearChaosAnimationProcedure;
import net.mcreator.jiukonmod.init.JiukcraftmodModEntities;

public class OnearChaosEntity extends Monster implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(OnearChaosEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(OnearChaosEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(OnearChaosEntity.class, EntityDataSerializers.STRING);
	...
	public OnearChaosEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(JiukcraftmodModEntities.ONEAR_CHAOS.get(), world);
	}

	public OnearChaosEntity(EntityType<OnearChaosEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
		setCustomName(Component.literal("Onear Chaos"));
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "onearchaos");
	}
	...
	public boolean hurt(DamageSource source, float amount) {
		OnearChaosAnimationProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this, source.getEntity());
	...
	public void die(DamageSource source) {
		super.die(source);
		OnearChaosQuandoAEntidadeMorreProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this, source.getEntity());
	}
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			...
			if (this.isDeadOrDying()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.model.death", EDefaultLoopTypes.PLAY_ONCE));
				return PlayState.CONTINUE;
			}
	...
	private <E extends IAnimatable> PlayState attackingPredicate(AnimationEvent<E> event) {
		...
		if (this.swinging && event.getController().getAnimationState().equals(software.bernie.geckolib3.core.AnimationState.Stopped)) {
			...
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.model.attack1", EDefaultLoopTypes.PLAY_ONCE));
			return PlayState.CONTINUE;
		}
		return PlayState.CONTINUE;
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 60) {
			this.remove(OnearChaosEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
